...
DATA = Path("data")
OUT = Path("outputs/exp007_multicrop_soundscapes")
OUT.mkdir(parents=True, exist_ok=True)

MULTICROP_META = Path("mels/train_audio_multicrop_metadata.csv")
...
def load_multicrop_train_audio_metadata():
    df = pd.read_csv(MULTICROP_META)
    df["source"] = "train_audio"
    df["filename"] = df["filename"].astype(str).str.strip()
    df["primary_label"] = df["primary_label"].astype(str).str.strip()
    df["secondary_labels"] = df["secondary_labels"].fillna("[]")
    df["mel_path"] = df["mel_path"].astype(str)
    return df
...
def main():
    ...
    # Multi-crop focal train_audio.
    train_audio_df = load_multicrop_train_audio_metadata()
    ...
    print("multicrop train_audio:", train_audio_df.shape)
    ...
    if "crop_type" in train_audio_df.columns:
        print("\nCrop counts:")
        print(train_audio_df["crop_type"].value_counts())
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        ckpt = {
            ...
            "experiment": "exp007_multicrop_soundscapes",
            "preprocessing": {
                "multicrop_meta": str(MULTICROP_META),
                "soundscape_meta": str(SOUNDSCAPE_META),
                "sr": SR,
                "duration": DURATION,
                "n_mels": N_MELS,
            },
