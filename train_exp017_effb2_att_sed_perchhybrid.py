...
DATA = Path("data")
OUT = Path("outputs/exp017_effb2_att_sed_perchhybrid")
...
class GeMFreqPool(nn.Module):
    """
    Generalized mean pooling over the frequency axis.
...
class EfficientNetB2AttSED(nn.Module):
    """
    Stronger SED head:
      EfficientNet-B2 backbone
      GeM frequency pooling
      dense bottleneck
      attention-weighted frame logits
      optional frame-max/attention blend
    """
    def __init__(
        self,
        num_classes,
    ...
    def forward(self, x):
        ...
        h = self.gem_freq(features)                       # [B, C, T]
        h = h.permute(0, 2, 1)                            # [B, T, C]
        h = self.dense(h)                                 # [B, T, hidden]
        h = h.permute(0, 2, 1)                            # [B, hidden, T]

        att_logits = self.att(h)                          # [B, classes, T]
        frame_logits = self.cla(h)                        # [B, classes, T]

        att = torch.softmax(torch.tanh(att_logits), dim=-1)
        att_clip_logits = torch.sum(att * frame_logits, dim=-1)

        max_logits = frame_logits.max(dim=-1).values

        return self.att_weight * att_clip_logits + self.max_weight * max_logits


def build_model(num_classes):
    return EfficientNetB2AttSED(
...
def main():
    ...
    pseudo_probs = np.load(PSEUDO_PROBS, mmap_mode="r")
    if pseudo_probs.shape != (len(pseudo_df), len(target_cols)):
        raise ValueError(
            f"pseudo_probs shape {pseudo_probs.shape} does not match "
            f"({len(pseudo_df)}, {len(target_cols)})"
        )

    trn = (
        pd.concat([train_audio_df, soundscape_df, pseudo_df], axis=0, ignore_index=True)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        ckpt = {
            ...
            "model_name": BACKBONE_NAME,
            "model_type": "efficientnet_b2_att_sed",
            "experiment": "exp017_effb2_att_sed_perchhybrid",
            "pseudo_loss_weight": PSEUDO_LOSS_WEIGHT,
            "pseudo_probs": str(PSEUDO_PROBS),
            "teacher": "exp015 Perch-hybrid pseudo labels; no Xeno-canto",
            "head": "GeM frequency pooling + attention pooling + frame max blend",
            "hidden_dim": 512,
            "att_weight": 0.5,
            "max_weight": 0.5,
            "dropout1": 0.25,
