...
def make_fake_sample_from_train_soundscape(target_cols):
    audio_files = sorted((DATA / "train_soundscapes").glob("*.ogg"))
    if not audio_files:
        raise FileNotFoundError("No train_soundscapes audio files found")

    stem = audio_files[0].stem

    rows = []
    for end_sec in [5, 10, 15]:
...
def main():
    real_sample = pd.read_csv(DATA / "sample_submission.csv")
    target_cols = real_sample.columns[1:].astype(str).tolist()
    sample = make_fake_sample_from_train_soundscape(target_cols)
